...
def categories_page(request):
    categories = Category.objects.all()  # Удалено .order_by('-created_at'), если такого поля нет
    context = {
        'categories': categories
    }
...
def guides_page(request):
  ...
  # Удалено .order_by('-created_at'), если такого поля нет
    context = {
        'guides': guides
    }
...
def jobs_by_category_page(request, slug):
    category = get_object_or_404(Category, slug=slug)  # Исправлено на category
    jobs = Job.objects.filter(category=category)  # Исправлено на category
    context = {
        'category': category,
        'jobs': jobs,
    }
